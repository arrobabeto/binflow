...
import { resolveEditImageProductionOrigin } from './edit-image-ingress.js';
...
export type ImagePreviewActions = Readonly<{
  approve: string;
  cancel: string;
}>;
...
const stageRequestState = (node: string): string => {
  switch (node) {
    case 'sync_editable_images':
    case 'validate_image_edit':
    case 'render_image_patch':
    case 'open_image_edit_pr':
      return 'GENERATING';
    case 'wait_preview':
...
export class ImageWorkflowRuntime {
  public constructor(
    private readonly database: Database,
    private readonly artifacts: ArtifactStore,
    private readonly executor: EditImageExecutor,
    private readonly orbitype: OrbitypeImagesPort,
  ...
  public async execute(raw: WorkflowResumeSignal): Promise<
    Readonly<{
      ...
      result: Readonly<{
        deployment: Awaited<
          ReturnType<EditImageExecutor['preparePreview']>
        >['deployment'];
  ...
  > {
    ...
    const context = await withSystemTenantScope(
      this.database,
      { operation: 'edit_image.execute.load', tenantId: signal.tenantId },
      async (database) => {
        ...
        if (row === undefined)
          throw new DomainError(
            'validation_error',
            'Edit image workflow context is missing.',
          );
        if (
          ...
          throw new DomainError(
            'conflict_error',
            'Edit image request is not executable.',
          );
        await database
        ...
        await database
          ...
          .set({
            currentNode: 'sync_editable_images',
    ...
    const parsedInput = editImageInputSchema.parse(
      context.version.interpretedInput,
    );
    if (parsedInput.mode !== 'execute')
      throw new DomainError(
        'validation_error',
        'Edit image execute input is required.',
      );

    const pages = await this.orbitype.listPages();
    const posts = await this.orbitype.listPosts();
    const candidate = resolveImageEditCandidate(
      pages,
      posts,
    ...
    try {
      imageBytes = await this.artifacts.get(
        parsedInput.replacementArtifactKey,
      );
    } catch {
      throw new DomainError(
        'validation_error',
        'Replacement image artifact is missing.',
        { code: 'image_replacement_missing' },
      );
    }

    const actions: ImagePreviewActions = Object.freeze({
      approve: randomBytes(24).toString('base64url'),
      cancel: randomBytes(24).toString('base64url'),
    });

    let preview: Awaited<ReturnType<EditImageExecutor['preparePreview']>>;
    const graphRunId = context.graph.id;
    const recordExecutionStage = async (node: string): Promise<void> => {
      await withSystemTenantScope(
        this.database,
        { operation: 'edit_image.execute.stage', tenantId: signal.tenantId },
        async (database) => {
    ...
    try {
      preview = await this.executor.preparePreview({
        ...
        mime: parsedInput.replacementMime,
        onStage: recordExecutionStage,
        orbitype: this.orbitype,
        productionOrigin: resolveEditImageProductionOrigin(
    ...
    await withSystemTenantScope(
      this.database,
      { operation: 'edit_image.execute.persist', tenantId: signal.tenantId },
      async (database) => {
        ...
        const patchKey = `${prefix}/image_edit_patch.json`;
  ...
  public async restorePreview(raw: WorkflowResumeSignal): Promise<void> {
    ...
    const context = await withSystemTenantScope(
      this.database,
      {
        operation: 'edit_image.restore_preview.load',
        ...
        const [row] = await database
          ...
          .innerJoin(
            ...
            and(
              ...
              eq(schema.artifacts.kind, 'image_edit_patch'),
            ),
          )
          .where(eq(schema.requests.id, signal.requestId))
          .limit(1);
    ...
    ) as ImageEditPatchArtifact;
    const preview = patch.orbitypePreview;
    if (preview === undefined || preview.restored === true) return;
    if (preview.applied !== true) return;

    try {
      await restoreOrbitypeImagePreview(this.orbitype, preview.restore);
    } catch (error) {
      await withSystemTenantScope(
        this.database,
        {
          operation: 'edit_image.restore_preview.fail',
          tenantId: signal.tenantId,
        },
        async (database) => {
          const terminal =
    ...
    const restoredPatch: ImageEditPatchArtifact = {
      ...patch,
    ...
    await withSystemTenantScope(
      this.database,
      {
        operation: 'edit_image.restore_preview.persist',
        tenantId: signal.tenantId,
      },
      async (database) => {
        const terminal =
  ...
  > {
    ...
    const context = await withSystemTenantScope(
      this.database,
      { operation: 'edit_image.publish.load', tenantId: signal.tenantId },
      async (database) => {
        const [row] = await database
          .select({
            artifact: schema.artifacts,
        ...
        if (
          ...
          throw new DomainError(
            'conflict_error',
            'Edit image request is not approved for publication.',
          );
        ...
        if (manifestRow === undefined)
          throw new DomainError(
            'validation_error',
            'Edit image manifest context is missing.',
          );
    ...
    if (patchObject === undefined)
      throw new DomainError(
        'validation_error',
        'Image edit patch artifact is missing.',
      );
    const patch = JSON.parse(
      new TextDecoder().decode(patchObject),
    ) as ImageEditPatchArtifact;
    ...
    const graphRunId = await withSystemTenantScope(
      this.database,
      { operation: 'edit_image.publish.graph', tenantId: signal.tenantId },
      async (database) => {
    ...
    let published: Awaited<ReturnType<EditImageExecutor['publish']>>;
    const recordPublishStage = async (node: string): Promise<void> => {
      await withSystemTenantScope(
        this.database,
        { operation: 'edit_image.publish.stage', tenantId: signal.tenantId },
        async (database) => {
          await this.recordStage(database, graphRunId, node);
        },
      );
    ...
    try {
      published = await this.executor.publish({
        ...
        orbitype: this.orbitype,
        patch,
        productionOrigin: resolveEditImageProductionOrigin(context.manifest),
    ...
    await withSystemTenantScope(
      this.database,
      { operation: 'edit_image.publish.persist', tenantId: signal.tenantId },
      async (database) => {
        await database
  ...
  private async recordFailure(
    ...
    await withSystemTenantScope(
      this.database,
      { operation: 'edit_image.fail', tenantId: signal.tenantId },
      async (database) => {
        const retryable =
            ...
            terminalResult: {
              ...
              errorMessage:
                error instanceof Error
                  ? error.message
                  : 'Edit image failed without an error message.',
              failureCode:
                error instanceof DomainError
