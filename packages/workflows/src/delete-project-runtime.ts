...
import { decideProjectDeletionPolicy } from '@binflow/policies';
...
export class DeleteProjectWorkflowRuntime {
  public constructor(
    private readonly database: Database,
    private readonly artifacts: ArtifactStore,
    private readonly executor: DeleteProjectExecutor,
    private readonly clock: Clock = systemClock,
  ) {}

  public async execute(raw: WorkflowResumeSignal): Promise<
    Readonly<{
      result: DeleteProjectExecutionResult;
    }>
  > {
    const signal = workflowResumeSignalSchema.parse(raw);
    const context = await withSystemTenantScope(
      this.database,
      { operation: 'delete_project.execute.load', tenantId: signal.tenantId },
      async (database) => {
        ...
        if (row === undefined)
          throw new DomainError(
            'validation_error',
            'Delete project workflow context is missing.',
          );
        if (
          ...
          throw new DomainError(
            'conflict_error',
            'Delete project request is not executable.',
          );
        await database
    ...
    try {
      result = await this.executor.execute({
        input: deleteProjectAstroInputSchema.parse(
          context.version.interpretedInput,
        ) as Extract<
          ReturnType<typeof deleteProjectAstroInputSchema.parse>,
          { mode: 'execute' }
        >,
    ...
    await withSystemTenantScope(
      this.database,
      { operation: 'delete_project.execute.persist', tenantId: signal.tenantId },
      async (database) => {
  ...
  > {
    ...
    const context = await withSystemTenantScope(
      this.database,
      { operation: 'delete_project.publish.load', tenantId: signal.tenantId },
      async (database) => {
        const [row] = await database
          .select({
        ...
        const policy = decideProjectDeletionPolicy({
    ...
    await withSystemTenantScope(
      this.database,
      { operation: 'delete_project.publish.start', tenantId: signal.tenantId },
      async (database) => {
    ...
    let published: Awaited<
      ReturnType<DeleteProjectExecutor['verifyProductionAbsence']>
    >;
    try {
      await withSystemTenantScope(
        this.database,
        { operation: 'delete_project.publish.stage', tenantId: signal.tenantId },
        async (database) => {
      ...
      await withSystemTenantScope(
        this.database,
        { operation: 'delete_project.publish.record_merge', tenantId: signal.tenantId },
        async (database) => {
          await database
      ...
      await withSystemTenantScope(
        this.database,
        { operation: 'delete_project.publish.verify', tenantId: signal.tenantId },
        async (database) => {
          const [run] = await database
            .select()
            .from(schema.graphRuns)
      ...
      if (resolvedSlug !== undefined) {
        await withSystemTenantScope(
          this.database,
          {
            operation: 'delete_project.publish.tombstone',
      ...
      await withSystemTenantScope(
        this.database,
        { operation: 'delete_project.publish.fail', tenantId: signal.tenantId },
        async (database) => {
          const retryable =
    ...
    await withSystemTenantScope(
      this.database,
      { operation: 'delete_project.publish.complete', tenantId: signal.tenantId },
      async (database) => {
        const now = this.clock.now();
        await database
          .update(schema.publicationAttempts)
  ...
  private async recordFailure(
    ...
    const errorCategory =
      error instanceof DomainError ? error.category : 'internal_error';
    const errorMessage =
      error instanceof Error ? error.message : 'Delete project failed.';
    await withSystemTenantScope(
      this.database,
      { operation: 'delete_project.execute.fail', tenantId: signal.tenantId },
      async (database) => {
        const [run] = await database
          .select()
          .from(schema.graphRuns)
