...
export type RevisionPlanActions = Readonly<{
  adjust: string;
  cancel: string;
  confirm: string;
}>;
...
export class BlogWorkflowRuntime {
  ...
  > {
    ...
    const recordExecutionStage = async (node: string): Promise<void> => {
      await withSystemTenantScope(
        this.database,
        { operation: 'blog.execute.stage', tenantId: signal.tenantId },
        async (database) => {
      ...
      result = await this.executor.execute({
        ...
        onTopicRefined: async (topic) => {
          await withSystemTenantScope(
            this.database,
            {
              operation: 'blog.execute.refine_topic',
              tenantId: signal.tenantId,
            },
            async (database) => {
              const interpreted = createBlogDraftInputSchema.parse(
                context.version.interpretedInput,
              );
              if (interpreted.mode !== 'brief') return;
              const nextInput = { ...interpreted, topic };
              ...
              await database
                ...
                .set({ interpretedInput: nextInput })
    ...
    await withSystemTenantScope(
      ...
      async (database) => {
        ...
        const bundleDigest = createHash('sha256')
          .update(bundleBytes)
          .digest('hex');
        const bundleKey = `${prefix}/blog_bundle.json`;
        await this.artifacts.put({
          bytes: bundleBytes,
          key: bundleKey,
  ...
  > {
    ...
    const context = await withSystemTenantScope(
      this.database,
      { operation: 'blog.interpret_revision.load', tenantId: signal.tenantId },
      async (database) => {
        const [row] = await database
          .select({
            graph: schema.graphRuns,
    ...
    const priorBundle = await this.loadBlogBundleArtifact(
      context.prior.id,
      context.request,
    );
    ...
    try {
      await withSystemTenantScope(
        this.database,
        {
          operation: 'blog.interpret_revision.stage',
          tenantId: signal.tenantId,
        },
        async (database) => {
          await this.recordStage(
    ...
    await withSystemTenantScope(
      this.database,
      { operation: 'blog.interpret_revision.persist', tenantId: signal.tenantId },
      async (database) => {
        const now = this.clock.now();
        await database
          .update(schema.requestVersions)
  ...
  > {
    ...
    const context = await withSystemTenantScope(
      this.database,
      { operation: 'blog.apply_revision.load', tenantId: signal.tenantId },
      async (database) => {
        const [row] = await database
          .select({
            graph: schema.graphRuns,
    ...
    const blogInput = createBlogDraftInputSchema.parse(
      context.version.interpretedInput,
    );
    const publicationDate =
      blogInput.mode === 'brief'
        ? (blogInput.publicationDate ??
          new Date().toISOString().slice(0, 10))
        : new Date().toISOString().slice(0, 10);
    const recordExecutionStage = async (node: string): Promise<void> => {
      await withSystemTenantScope(
        this.database,
        { operation: 'blog.apply_revision.stage', tenantId: signal.tenantId },
        async (database) => {
    ...
    await withSystemTenantScope(
      this.database,
      { operation: 'blog.apply_revision.persist', tenantId: signal.tenantId },
      async (database) => {
        const now = this.clock.now();
        for (const file of outcome.files) {
          await database.insert(schema.artifacts).values({
        ...
        await database
          ...
          .set({
            ...
            terminalResult: {
              ...
              branch: outcome.publication.branch,
              bundleDigest,
              categoryKind: outcome.bundle.categoryKind,
              files: outcome.publication.files,
  ...
  > {
    ...
    let published: Awaited<ReturnType<BlogExecutor['verifyProduction']>>;
    try {
      await withSystemTenantScope(
        this.database,
        { operation: 'blog.publish.stage', tenantId: signal.tenantId },
        async (database) => {
  ...
  private async loadBlogBundleArtifact(
    requestVersionId: string,
    request: Pick<
    ...
    const artifact = await withSystemTenantScope(
      this.database,
      { operation: 'blog.load_bundle', tenantId: request.tenantId },
      async (database) => {
          ...
          const [row] = await database
            ...
            .where(
              and(
                ...
                eq(schema.artifacts.kind, 'blog_bundle'),
              ),
            )
            .limit(1);
        ...
        throw new DomainError(
          'conflict_error',
          'Prior blog bundle artifact is missing.',
        );
      },
    );
    const bytes = await this.artifacts.get(artifact.storageKey);
    return adaptedGeneratedBlogBundleSchema.parse(
  ...
  private async loadCoverImageArtifact(
    ...
    const artifact = await withSystemTenantScope(
      this.database,
      { operation: 'blog.load_cover', tenantId: request.tenantId },
      async (database) => {
        let cursor: string | null = requestVersionId;
        const seen = new Set<string>();
        while (cursor !== null && !seen.has(cursor)) {
