#!/usr/bin/env node
/**
 * Append edit_text_style@1 to the active Bistro (astro_orbitype) manifest bindings.
 *
 * Usage:
 *   DATABASE_URL=postgresql://binflow:binflow_local@localhost:5432/binflow \
 *     pnpm --filter @binflow/tools exec tsx scripts/add-bistro-edit-text-style-binding.ts
 */
...
const editTextStyleBinding = Object.freeze({
  access: 'client_publish' as const,
  capabilityId: 'edit_text_style' as const,
...
const patchManifestDocument = (document: ProjectManifest): ProjectManifest => {
  const hasBinding = document.enabledCapabilities.some(
    (binding) => binding.capabilityId === 'edit_text_style',
  );
  const enabledCapabilities = hasBinding
    ? document.enabledCapabilities
    : [...document.enabledCapabilities, editTextStyleBinding];
    ...
    fingerprint: createHash('sha256')
      .update(
        JSON.stringify({
          ...
          reason: 'add-bistro-edit-text-style-binding',
        }),
      )
      .digest('hex'),
  });
...
const result = await withPlatformOwnerScope(
  database,
  {
    actorId: 'script:add-bistro-edit-text-style-binding',
    correlationId: `add-bistro-edit-text-style-binding:${Date.now()}`,
    reason: 'Enable edit_text_style on Bistro manifest',
  },
  async (scoped) => {
    ...
    await scoped.insert(schema.projectManifestVersions).values({
      createdBy: 'script:add-bistro-edit-text-style-binding',
    ...
    await scoped.insert(schema.projectCapabilityBindings).values({
      access: editTextStyleBinding.access,
      capabilityId: editTextStyleBinding.capabilityId,
      capabilityVersion: editTextStyleBinding.capabilityVersion,
      createdBy: 'script:add-bistro-edit-text-style-binding',
      id: uuidv7(),
      manifestVersionId: patched.id,
      projectId: bistro.projectId,
