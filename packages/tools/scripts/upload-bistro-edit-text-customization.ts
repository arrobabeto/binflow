#!/usr/bin/env node
/**
 * Upload docs/customizations/bistro-edit-text.md into the local Bistro project.
 *
 * Usage:
...
const body = await readFile(
  join(root, 'docs/customizations/bistro-edit-text.md'),
  'utf8',
);
...
const projects = await withPlatformOwnerScope(
  database,
  {
    actorId: 'script:upload-bistro-edit-text-customization',
    correlationId: 'upload-bistro-edit-text-customization',
    reason: 'Resolve bistro project for edit_text customization upload',
  },
  async (scoped) =>
    scoped
      .select({
...
const summary = await catalog.uploadCustomization(
  ...
  {
    actorId: 'script:upload-bistro-edit-text-customization',
    correlationId: `upload-bistro-edit-text-customization:${Date.now()}`,
