#!/usr/bin/env node
/**
 * Upload docs/customizations/webbin-create-project-astro.md into the local
...
const body = await readFile(
  join(root, 'docs/customizations/webbin-create-project-astro.md'),
  'utf8',
);
...
const projects = await withPlatformOwnerScope(
  database,
  {
    actorId: 'script:upload-webbin-project-customization',
    correlationId: 'upload-webbin-project-customization',
    reason: 'Resolve webbin project for customization upload',
  },
  async (scoped) =>
    scoped
      .select({
...
const summary = await catalog.uploadCustomization(
  ...
  {
    actorId: 'script:upload-webbin-project-customization',
    correlationId: `upload-webbin-project-customization:${Date.now()}`,
